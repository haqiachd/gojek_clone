...
class GojekBottomAppbar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    BottomNavBloc bloc = BlocProvider.of<BottomNavBloc>(context);
    return Container(
        ...
        child: Stack(
          children: [
            BlocBuilder<BottomNavBloc, int>(
              builder: (context, posisi) => AnimatedPositioned(
                duration: const Duration(milliseconds: 1000),
                top: 0,
                left: _getWithContent(context) * posisi,
                  ...
                  ...bottomNavData.map(
                    (data) => InkWell(
                      onTap: () {
                        ...
                        switch (data.label) {
                          case BottomNavModel.beranda:
                            _posTop = 0;
                            bloc.add(BottomNavEvent.main_bottom_nav_beranda);
                            break;
                          case BottomNavModel.promo:
                            _posTop = 1;
                            bloc.add(BottomNavEvent.main_bottom_nav_promo);
                            break;
                          case BottomNavModel.pesanan:
                            bloc.add(BottomNavEvent.main_bottom_nav_pesanan);
                            _posTop = 2;
                            break;
                          case BottomNavModel.chat:
                            bloc.add(BottomNavEvent.main_bottom_nav_chat);
                            _posTop = 3;
                            break;
                        }
                        // },
                      ...
                      child: BlocBuilder<BottomNavBloc, int>(
                        builder: (context, posisi) => Container(
                          width: _getWithContent(context),
                          height: 60,
                          decoration: BoxDecoration(
                            color: Colors.white,
